...
class LSTMRNN(object):
    ...
    def add_input_layer(self,):
        ...
        Ws_in1 = self._weight_variable([self.input_size, self.cell_size], name='w1')
        bs_in1 = self._bias_variable([self.cell_size, ], name='b1')
        Ws_in2 = self._weight_variable([self.cell_size, self.cell_size], name='w2')
        bs_in2 = self._bias_variable([self.cell_size, ], name='b2')
    ...
    def add_cell(self):
        ...
        stacked_cell = tf.nn.rnn_cell.MultiRNNCell([res_cell] * CELL_DEEP)
        _, self.cell_final_state = tf.nn.dynamic_rnn(stacked_cell, inputs=self.l_in_y, dtype=tf.float32, time_major=False)

    def add_output_layer(self):
        # shape = (batch, cell_size)
        l_out_x1 = tf.reshape(self.cell_final_state[-1], [-1, self.cell_size], name='2_2D1')
        Ws_out1 = self._weight_variable([self.cell_size, self.cell_size], name='ow1')
        bs_out1 = self._bias_variable([self.cell_size, ], name='ob1')
        Ws_out2 = self._weight_variable([self.cell_size, self.output_size], name='ow2')
        bs_out2 = self._bias_variable([self.output_size, ], name='ob2')
