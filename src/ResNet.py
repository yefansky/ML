...
class LSTMRNN(object):
    ...
    def getLSTMCell(self, n, res=False):
        lstm_cell = tf.nn.rnn_cell.LSTMCell(n, forget_bias=1, initializer=tf.orthogonal_initializer(), state_is_tuple=True)
        if res:
            return tf.nn.rnn_cell.ResidualWrapper(cell=lstm_cell)
        else:
            return tf.nn.rnn_cell.DropoutWrapper(cell=lstm_cell, input_keep_prob=0.2, output_keep_prob=0.2)

    def add_cell(self):
        cells = [self.getLSTMCell(self.cell_size, True), self.getLSTMCell(32), self.getLSTMCell(32, True), self.getLSTMCell(32, True), self.getLSTMCell(32, True), self.getLSTMCell(32, True), self.getLSTMCell(self.cell_size)]
